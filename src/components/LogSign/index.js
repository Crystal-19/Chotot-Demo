...
const renderInputSection = ({password, buttonContent, text, path, link, action}) => {
  return (
    <>
...
const LogSign = ({title, content, password, buttonContent, text, link, action, path}) => {
  return (
    <div className="background-img">
      <div className="general-container structure-container">
        <div className="log-sign-container">
          {renderLogSignTitle({title, content})}
          {renderInputSection({password, buttonContent, text, path, link, action})}
        </div>
      </div>
      <Footer />
    </div>
